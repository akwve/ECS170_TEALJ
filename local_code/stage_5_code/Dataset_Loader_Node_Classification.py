...
class Dataset_Loader(dataset):
    ...
    def load(self):
        ...
        dataset_name = str(self.dataset_name).lower().strip()
        ...
        if dataset_name == 'cora':
            ...
            train_end = max(1, int(num_nodes * 0.05))
            val_end = max(train_end + 1, int(num_nodes * 0.15))
            idx_train = range(0, train_end)
            idx_val = range(train_end, val_end)
            idx_test = range(val_end, num_nodes)
