...
from local_code.stage_3_code.Method_CNN_MNIST import CNN_MNIST
...
if 1:
    ...
    np.random.seed(2)
    torch.manual_seed(2)
    config = {'max_epoch': 30, 'learning_rate': 1e-3, 'batch_size': 128, 'num_filters_1': 16, 'num_filters_2': 32, 'kernel_size': 3}

    #------------------------------------------------------

    # ---- objection initialization setction ---------------
    data_obj = Dataset_Loader('MNIST', 'MNIST digit recognition dataset')
    data_obj.dataset_source_folder_path = '../../data/stage_3_data/MNIST'

    evaluate_obj = Evaluate_Accuracy('evaluating_metrics', '')
    method_obj = CNN_MNIST('CNN_MNIST', 'CNN for MNIST dataset')
    result_obj = Result_Saver('result_saver', '')

    result_obj.result_destination_folder_path = '../../result/stage_3_result/'
    result_obj.result_destination_file_name = 'prediction_result_MNIST'
    ...
    plot_file_path = '../../result/stage_3_result/training_loss_curve_mnist.png'
    save_training_loss_curve(
        [method_obj.training_loss_history],
        plot_file_path,
        'MNIST Training Loss Convergence',
        line_color='tab:blue'
    )
