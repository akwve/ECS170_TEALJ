...
if __name__ == '__main__':
    ...
    data_obj = Dataset_Loader('citeseer', 'Citeseer citation network node classification')
    data_obj.dataset_source_folder_path = str(PROJECT_ROOT / 'data' / 'stage_5_data' / 'citeseer')

    evaluate_obj = Evaluate_Accuracy('evaluating_metrics', '')
    method_obj = Method_GCN('GCN_Node_Classification_Citeseer', 'GCN for citeseer node classification')
    ...
    plot_file_path = str(result_folder / 'gcn_learning_curves_citeseer.png')
    save_training_loss_curve([method_obj.training_loss_history], plot_file_path, 'GCN Training Loss on Citeseer')
